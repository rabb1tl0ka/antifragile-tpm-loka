...
```
    system_msg = (
        ...
        "STYLE:\n"
        "- Phrase each bullet in natural coaching language, like guidance from a senior TPM.\n"
        "- You may briefly explain why the mistake is harmful, but keep it concise (1–2 short sentences max).\n"
        "HARD RULES:\n"
        "- Use the CONTEXT only for reasoning; never copy or quote it.\n"
        "- No preamble, no title, no numbering, no examples, no extra sections.\n"
        "- Do not offer positive advice or instructions — only frame guidance as mistakes to avoid.\n"
        "QUALITY CHECK:\n"
        "- Before finalizing, rewrite any bullet that does not start with 'Do not '.\n"
        "- Remove any wording that prescribes actions to take.\n"
    )
...
DO NOT start project work without confirming all key stakeholders share the same definition of success.
DO NOT launch without stress-testing critical flows.
DO NOT skip validating source data at project start.
DO NOT schedule core ceremonies in one timezone only.
DO NOT assume compliance review is trivial. [end of text]
...
Never copy text from between CONTEXT_START and CONTEXT_END.
Respond with 3–5 bullet points, each starting with 'Do not '.
Output ONLY the bullets. Do NOT include any other information (e.g., context).
